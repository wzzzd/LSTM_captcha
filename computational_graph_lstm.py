...
from captcha_test.captcha_lstm.captcha_read import *
from captcha_test.captcha_lstm.config import *



def computational_graph_lstm(x, y):
    ...
    prediction_all = tf.reshape(prediction_all,[batch_size, captcha_num, char_set_len]) # [4, batch, num_class] => [batch, 4, num_class]

    #loss_function
    loss = -tf.reduce_mean(y * tf.log(prediction_all))
    # loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=prediction_all,labels=y))
    #optimization
    opt = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss)
    #model evaluation
    pre_arg = tf.argmax(prediction_all,2)
    y_arg = tf.argmax(y,2)
